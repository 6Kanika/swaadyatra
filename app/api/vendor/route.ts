...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Convert images to Resend attachments ──────────────────────────────────
    ...
    const result = await resend.emails.send({
      ...
      subject: `[SwaadYatra] New Food Spot Listing — ${foodSpotName}, ${city}`,
      attachments,
      html: `
          ...
          <div style="padding:32px;background:#fff;">
            <table style="width:100%;border-collapse:collapse;font-size:15px;">
              <tr>
                ...
                <td style="padding:10px 0;color:#222;">${vendorName}</td>
              </tr>
              <tr style="border-top:1px solid #f0f0f0;">
                <td style="padding:10px 0;color:#888;vertical-align:top;font-weight:600;">Food Spot Name</td>
                <td style="padding:10px 0;color:#222;">${foodSpotName}</td>
              </tr>
              <tr style="border-top:1px solid #f0f0f0;">
    ...
    console.log("[vendor/route] Resend result:", JSON.stringify(result));
